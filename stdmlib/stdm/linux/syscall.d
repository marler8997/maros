/**
 NOTE: I add the "sys_" prefix to all these functions to distinguish them
       ...
       do not clash.
*/
module stdm.linux.syscall;

import stdm.sentinel : SentinelPtr;
import stdm.c : cstring;
import stdm.linux.cthunk : kernel, mode_t, off_t, gid_t, uid_t;
import stdm.linux.file : FileD, stat_t, OpenFlags, AccessMode;
import stdm.linux.filesys : linux_dirent;
import stdm.linux.signals : siginfo_t, sigaction_t;
import stdm.linux.process : pid_t, idtype_t, rusage;
import stdm.linux.capability : cap_user_header_t, cap_user_data_t;
...
extern (C) FileD sys_open(cstring pathname, OpenFlags flags)
{
    mixin(passthroughSyscall);
}
