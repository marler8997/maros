//
// I've copied this from zig's std library to fix it
// There's a problem with section ordering
//
...
const BinaryElfOutput = struct {
    ...
    pub fn parse(allocator: *Allocator, name: []const u8, elf_file: File) !Self {
        var self: Self = .{
        ...
        while (try section_headers.next()) |section| {
            if (sectionValidForOutput(section)) {
                ...
                std.log.info("{s}: section 0x{x:0>8} - 0x{x:0>8} ({} bytes)", .{name,
                    newSection.elfOffset, newSection.elfOffset + newSection.fileSize,
                    newSection.fileSize});
        ...
        while (try program_headers.next()) |phdr| {
            if (phdr.p_type == elf.PT_LOAD) {
                ...
                std.log.info("{s}: segment 0x{x:0>8} - 0x{x:0>8} phaddr=0x{x} vaddr=0x{x}", .{name,
                    newSegment.elfOffset, newSegment.elfOffset + newSegment.fileSize,
                    newSegment.physicalAddress, newSegment.virtualAddress});
                for (self.sections.items) |section| {
                    if (sectionWithinSegment(section, phdr)) {
                        std.log.info("{s}:     got section @0x{x:0>8}", .{name, section.elfOffset});
        ...
        std.log.info("{s}: sorted segments", .{name});
        for (self.segments.items) |segment| {
            std.log.info("{s}: 0x{x:0>8}", .{name, segment.elfOffset});
        }

        for (self.segments.items) |first_segment, i| {
            if (first_segment.firstSection) |firstSection| {
                const diff = firstSection.elfOffset - first_segment.elfOffset;

                first_segment.elfOffset += diff;
                first_segment.fileSize += diff;
                first_segment.physicalAddress += diff;

                const basePhysicalAddress = first_segment.physicalAddress;

                for (self.segments.items[i+1..]) |next_segment| {
                    next_segment.binaryOffset = next_segment.physicalAddress - basePhysicalAddress;
                }
                break;
            }
        }
        ...
        std.log.info("{s}: sorted sections:", .{name});
        for (self.sections.items) |section| {
            std.log.info("{s}: section 0x{x:0>8} - 0x{x:0>8} ({} bytes) binary_offset=0x{x:0>8}", .{name,
                section.elfOffset, section.elfOffset + section.fileSize,
                section.fileSize, section.binaryOffset});
        }
...
fn writeBinaryElfSection(elf_file: File, out_file: File, section: *BinaryElfSection) !void {
    ...
    std.log.info("writeBinaryElfSection 0x{x:0>8}", .{section.elfOffset});
...
fn emitRaw(allocator: *Allocator, elf_path: []const u8, raw_path: []const u8, format: RawFormat) !void {
    ...
    var binary_elf_output = try BinaryElfOutput.parse(allocator, std.fs.path.basename(elf_path), elf_file);
    defer binary_elf_output.deinit();

    switch (format) {
        .bin => {
            for (binary_elf_output.sections.items) |section| {
                std.log.info("write section offset={} binoff={} size={}", .{
                    section.elfOffset, section.binaryOffset, section.fileSize});
                    // TODO: segment?
