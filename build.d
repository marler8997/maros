...
import stdm.sentinel : makeSentinel;
import stdm.format : formatHex;
import stdm.c;
import stdm.ctypes : off_t, mode_t;
import stdm.conv : tryParseEnum;
import stdm.typecons : Nullable;
import stdm.file : getFileSize, tryGetFileMode, fileExists,
    S_IXOTH,
    S_IWOTH,
    S_IROTH,
    S_IRWXO,
...
import stdm.filesys : mkdir;
import mbr = stdm.disk.mbr;
// TODO: replace this linux-specific import
//import stdm.linux.file : open, close;
import stdm.linux.capability : CAP_TO_MASK, CAP_SYS_ADMIN, CAP_SYS_CHROOT;
...
{
    import stdm.file;
    print(stdout, "[BUILD] ", args, "\n");
}
void logError(T...)(T args)
{
    import stdm.file;
    print(stdout, "Error: ", args, "\n");
}
...
void logMkdir(string pathname, mode_t mode = DefaultDirMode)
{
    ...
    /*
        ...
        {
            print(stderr, "Error: expected path \"", pathnameCStr, "\" mode to be 0x",
...
Command("buildUser", "build userspace of the os", cmdInSequence, function(string[] args)
{
    ...
    string compiler = "dmd" ~ CompilerArgs()
        .version_("NoStdc")
        .conf("")
        .betterC
        //.inline
    ...
    case Mode.release: compiler ~= " -release -O"; break;
    }
    ...
    const stdmlibPath = "stdmlib";

    string stdmSrc(string path)
    {
        return stdmlibPath ~ "/stdm/" ~ path;
    }
...
Command("allocImage", "allocate a file for the disk image", cmdInSequence, function(string[] args)
{
    ...
    run(format("qemu-img create %s %s", config.imageFile.formatFile,
        config.imageSize.formatQemu));
    return 0;
}),
...
Command("installBootloader", "install the bootloader", cmdInSequence, function(string[] args)
{
    ...
    log("Succesfully installed the '", config.bootloader, "' bootloader");
    return 0;
}),
...
Command("installKernel", "install the kernel to the rootfs", cmdInSequence, function(string[] args)
{
    ...
    if (config.bootloader == Bootloader.crystal)
    {
...
Command("startQemu", "start the os using qemu", cmdNoFlags, function(string[] args)
{
...

    {
        ...
        case SerialSetting.stdio   : extraArgs ~= " -serial stdio"; break;
        case SerialSetting.file    : extraArgs ~= " -serial file:serial.log"; break;
        case SerialSetting.telnet  : extraArgs ~= " -serial telnet:0.0.0.0:1234,server,wait"; break;
        }
    }

    run(format("qemu-system-x86_64 -drive format=raw,file=%s%s", config.imageFile, extraArgs));
    return 0;
}),
...
Command("readMbr", "read/print the MBR of the image using the stdm library", cmdNoFlags, function(string[] args)
{
    import std.stdio;
